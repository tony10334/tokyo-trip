...
- 後端是 textdb.online（免註冊、免金鑰）。⚠️ 原本用 jsonblob.com,
  但 2026/8 發現它把「建立新 blob」關了（POST 一律 403）,只好換。
  textdb 的 key 寫入即建立,所以房間 30 天過期後會在下次寫入時自動復活,
  app 開啟時若雲端是空的也會自動把本機資料推回去（Store.init）
- textdb 限制：單筆 20 萬字元、每 IP 每天 500 次寫入、讀取不限
